...
public partial class MeshChangerSystem : SystemBase
{
    ...
    protected override void OnStartRunning()
    {
        ...
        foreach (var changer in SystemAPI.Query<MeshChanger>())
        {
    ...
    {
        foreach (var (changer, mmi) in SystemAPI.Query<MeshChanger, RefRW<MaterialMeshInfo>>())
        {
            ...
            if (changer.frame >= changer.frequency)
            {
                ...
                mmi.ValueRW.MeshID = m_MeshMapping[mesh];
            }
        }
    }
}
